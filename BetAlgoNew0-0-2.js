...
const maxWinField = document.getElementById('max-win-field')
...
const percentReturnText = document.getElementById('percent-return-text')
const guaranteedWinAmountText = document.getElementById('guaranteed-win-amount-text')

//Global Variables
var matchupDict = {}
var betTypeDict = {}
var bettingPower = 500
var maxWin = 500
var numParlays = 0
var totalAction = 0
...
maxWinField.addEventListener('blur', () => {
    maxWin = parseFloat(maxWinField.value)
})
...
maxWinField.addEventListener('focus', () => {
    maxWinField.placeholder = ''
})
...
function buildParlays(parlayArray) {
    ...
    parlayArray.forEach( (parlay, index) => {
        ...
        var betAmountText = createDOMElement('div', 'bet-amount-text', '', payoutDiv)
        betAmountText.setAttribute('id', `bet-amount-text-${index+1}`)
        var winAmountText = createDOMElement('div', 'bet-amount-text', '', payoutDiv)
        winAmountText.setAttribute('id', `win-amount-text-${index+1}`)

        calculateOddsMultiplier(parlay)
...
function calculateAdjustedBets() {
    var sumBets = 0
